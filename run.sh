...
# 背景图辅助类（static Image 单次加载，避免每帧 Image.createImage()）
GENERATE_HELPER_BG() {
cat > ResizeBgHelper.java << HELPEOF
import javax.microedition.lcdui.Graphics;
import javax.microedition.lcdui.Image;

public class ResizeBgHelper {
    ...
    public static void drawBg(Graphics g) {
        if (bgImage != null) {
            g.drawImage(bgImage, 0, 0, ANCHOR);
        }
    }
}
HELPEOF
...
# ASM 类版本回写工具（将 class 版本号对齐为参考版本）
# 第1个参数为目标主版本号（如 52），后续参数为要回写的 .class 文件
# 使用游戏自身 class 版本号作参考，确保真机和模拟器均兼容
GENERATE_PATCHER() {
cat > ClassVersionPatcher.java << PATCHEREOF
import java.io.*;

public class ClassVersionPatcher {
    public static void main(String[] args) throws IOException {
        int targetVer = Integer.parseInt(args[0]);
        for (int i = 1; i < args.length; i++) {
            ...
            if (data.length < 8 ||
                ...
                System.err.println("  [!] 非法 class 文件: " + file);
                continue;
            }
            int oldVer = ((data[6] & 0xFF) << 8) | (data[7] & 0xFF);
            data[6] = (byte)(targetVer >> 8);
            data[7] = (byte)(targetVer);
            writeAll(file, data);
            System.out.println("  [ver] " + file + " -> v" + oldVer + " -> v" + targetVer);
        }
    }
    private static byte[] readAll(String file) throws IOException {
    ...
    private static void writeAll(String file, byte[] data) throws IOException {
        try (FileOutputStream fos = new FileOutputStream(file)) {
            fos.write(data);
        }
    }
...
# ★ StripFrames: 用 ASM SKIP_FRAMES + COMPUTE_MAXS 擦除 class 的所有栈帧属性
#   输入: class 文件路径（就地修改）
#   用途: ResizeBgHelper 等 javac 编译的 class 自带 StackMapTable，
#         注入 JAR 前需擦除以免老模拟器的 ASM 解析崩溃
GENERATE_STRIP_FRAMES() {
cat > StripFrames.java << STRIPEOF
import org.objectweb.asm.*;
import java.io.*;

public class StripFrames {
    public static void main(String[] args) throws IOException {
        for (String file : args) {
            byte[] in = readAll(file);
            ClassReader cr = new ClassReader(in);
            // COMPUTE_MAXS 只算 max_stack/max_locals，不生成任何栈帧
            ClassWriter cw = new ClassWriter(ClassWriter.COMPUTE_MAXS);
            // SKIP_FRAMES 丢弃输入中所有 StackMap/StackMapTable
            cr.accept(new ClassVisitor(Opcodes.ASM4, cw) {}, ClassReader.SKIP_FRAMES);
            try (FileOutputStream fos = new FileOutputStream(file)) {
                fos.write(cw.toByteArray());
            }
            System.out.println("  [strip] " + file + " 栈帧已擦除");
        }
    }
    private static byte[] readAll(String file) throws IOException {
        File f = new File(file);
...
# 清理函数（含 ResizeHook + ResizeBgHelper + ClassVersionPatcher + StripFrames）
CLEANUP_HOOK() {
    rm -f ResizeHook*.java ResizeHook*.class ResizeBgHelper*.java ResizeBgHelper*.class ClassVersionPatcher*.java ClassVersionPatcher*.class StripFrames*.java StripFrames*.class 2>/dev/null || true
}
...
# 构建 classpath（OpenJDK 8 + ASM 4.0 + CLDC 1.1 + MIDP 2.0）
CP_ALL="${ASM_JAR}:${CLDC_JAR}:${MIDP_LIB}"
echo -e "${GREEN}[√] J2ME 编译环境: OpenJDK 8 + ASM 4.0 + CLDC 1.1 + MIDP 2.0${RESET}"
...
case "${MODE}" in
    ...
    2)
        ...
        # 背景图模式需要 J2ME 标准库编译 ResizeBgHelper
        # 依赖校验已在前面完成，此处不再重复
        ;;
    *)
...
# 检测游戏 class 版本号（python3 > xxd > 默认48）
# ★ 必须在步骤计数之前，因为 v45 游戏跳过 preverify 会影响总步骤数
GAME_VER=48
GAME_CLS=$(find "${TMP_DIR}" -name "*.class" 2>/dev/null | head -1)
if [ -n "${GAME_CLS}" ]; then
    GAME_VER=$(python3 -c "import struct,sys; print(struct.unpack('>H', open(sys.argv[1],'rb').read()[6:8])[0])" "${GAME_CLS}" 2>/dev/null)
    if [ -z "${GAME_VER}" ] || [ "${GAME_VER}" -lt 45 ] 2>/dev/null; then
        GAME_VER_HEX=$(xxd -p -s6 -l2 "${GAME_CLS}" 2>/dev/null || hexdump -s6 -n2 -e '"%04x"' "${GAME_CLS}" 2>/dev/null)
        if [ -n "${GAME_VER_HEX}" ]; then
            GAME_VER=$((16#${GAME_VER_HEX}))
            [ "${GAME_VER}" -lt 45 ] 2>/dev/null && GAME_VER=48
        fi
    fi
fi
echo -e "      [√] 游戏 class 版本: v${GAME_VER}"

# 计算总步骤数（无 ProGuard，无预验证）
# ASM SKIP_FRAMES 已确保 class 无栈帧，兼容 kemnnx64 / 老顽童
if [ "${HOOK_MODE}" = "bg" ]; then
...
if [ "${HOOK_MODE}" = "bg" ]; then
    ...
    GENERATE_PATCHER
    GENERATE_STRIP_FRAMES
    echo -e "      ${GREEN}[√] ResizeHook.java + ResizeBgHelper.java + ClassVersionPatcher.java + StripFrames.java 生成成功${RESET}"
else
    GENERATE_HOOK_BASIC
...
if [ "${HOOK_MODE}" = "bg" ]; then
    ...
    echo -e "      [√] 游戏 class 版本: v${GAME_VER} (ResizeBgHelper 将对齐此版本)"
    
    # ★ 编译后处理：擦除 javac 生成的 StackMapTable + 对齐版本号
    #   1) StripFrames: 去除 StackMapTable（避免旧 ASM readFrameType 崩溃）
    #   2) ClassVersionPatcher: 对齐游戏版本（KEmulator CustomClassAdapter 要求一致）
    java -cp ".:${CP_ALL}" StripFrames ResizeBgHelper.class
    java -cp ".:${CP_ALL}" ClassVersionPatcher "${GAME_VER}" ResizeBgHelper.class
    echo -e "      [√] ResizeBgHelper.class → v${GAME_VER} (无栈帧)"
fi
...
# 背景图版：注入静态辅助类到 JAR（已在编译步骤擦除栈帧+对齐版本）
if [ "${HOOK_MODE}" = "bg" ]; then
    cp ResizeBgHelper.class "${TMP_DIR}/"
    echo -e "      ${GREEN}[√] ResizeBgHelper.class 已注入 (v${GAME_VER}, 无栈帧)${RESET}"
fi
...
# ★ SKIP_FRAMES 策略说明:
#   - 所有 class 已通过 ASM4 SKIP_FRAMES + COMPUTE_MAXS 擦除栈帧
#   - KEmulator / kemnnx64 / 老顽童: 无 StackMap 时走完整字节码验证 ✓
#   - OpenJDK 8 + ASM 4.0 编译环境确保 v45.3/v46.0 兼容
#   - 不需要 ProGuard / preverify
